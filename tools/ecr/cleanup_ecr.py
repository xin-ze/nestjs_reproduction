...
image_list = [
    "serverless/car",
    "serverless/refi-engine",
    "serverless/remote-config-service",
    "serverless/auth",
    "serverless/content",
    "serverless/crm-service",
    "serverless/messaging",
    "serverless/post-bind-service",
    "serverless/seo-payload",
    "serverless/signup-service",
    "serverless/account",
    "serverless/garage",
    "serverless/order-service",
    "serverless/quoting-service",
    "serverless/telematics",
    "serverless/commission-statement",
]
...
def clean_ecr_images():
    ...
    repositories = json.loads(
        subprocess.check_output(["aws", "ecr", "describe-repositories"])
    )
...
def delete_ecr_image_by_digests(repo_name, digests):
    ...
    while k < l:
        ...
        print(
            f"aws ecr batch-delete-image --repository-name {repo_name} --image-ids imageDigest={ptag}"
        )
...
def delete_ecr_image_by_tags(repo_name, tags, tagType):
    ...
    while k < l:
        ...
        print(
            f"aws ecr batch-delete-image --repository-name {repo_name} --image-ids imageTag={ptag}, tag type {tagType}"
        )
        res = subprocess.Popen(
            f"aws ecr batch-delete-image --repository-name {repo_name} --image-ids imageTag={ptag}",
            shell=True,
            stdout=subprocess.PIPE,
        ).stdout.read()
        print(res)
